...
class PointPillarFFNet(PointPillarWhere2comm):
    ...
    def extract_bev_feature_384(self, data_dict):
        """
        提取 384 维特征（VFE + Scatter + Backbone）
        用于 Loss 计算的 GT
        """
        batch_dict = self.pillar_vfe(data_dict)
        batch_dict = self.scatter(batch_dict)
        batch_dict = self.backbone(batch_dict)
        return batch_dict['spatial_features_2d']  # [B, 384, H, W]
    
    def extract_bev_feature_64(self, data_dict):
        """提取 64 维特征（VFE + Scatter）"""
        batch_dict = self.pillar_vfe(data_dict)
        batch_dict = self.scatter(batch_dict)
        return batch_dict['spatial_features']  # ← 改为 'spatial_features'
    ...
    def forward(self, data_dict):
        ...
        if 'ffnet_t0' in data_dict.keys():
            ...
            # # 提取历史特征 (384维)
            # with torch.no_grad():
            #      feat_t0_64 = self.extract_bev_feature_64(ffnet_t0_dict) 
            #      feat_t1_64 = self.extract_bev_feature_64(ffnet_t1_dict) 

            # # 生成 Flow
            # flow_pred = self.flow_generator(feat_t0_64, feat_t1_64)
            # with torch.no_grad():
            #     feat_t1_384 = self.extract_bev_feature_384(ffnet_t1_dict)
            # 并行提取特征
            with torch.no_grad():
            ...
            # 预测 t2
            feat_pred_t2 = feat_t1_384 + flow_pred / (dt_01 + 1e-6) * dt_12
            feat_gt_t2 = spatial_features_2d[1:: 2].clone()  # [B, 384, H, W]
