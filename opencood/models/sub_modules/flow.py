...
class FlowGenerator(nn.Module):
    def __init__(self, args):
        ...
        import copy
        backbone_cfg = copy.deepcopy(args['backbone'])
        backbone_cfg['in_channels'] = feature_dim * 2

        if 'resnet' in backbone_cfg:
            self.flow_backbone = ResNetBEVBackbone(backbone_cfg,
                                                   feature_dim * 2)
        else:
            self.flow_backbone = BaseBEVBackbone(backbone_cfg,
                                                 feature_dim * 2)
        self.reduce_conv = nn.Conv2d(
            feature_dim * 2,   # concat 后
            feature_dim,       # backbone 期望
            kernel_size=1
        )
        c = 384
         # Flow Prediction Head
        self.flow_head = nn.Sequential(
            nn.Conv2d(c, 128, kernel_size=3, padding=1), # 384->192
            nn.ReLU(inplace=True),
            nn.Conv2d(128, 2, kernel_size=1)           # 48->2
        )
        # 初始化最后一层卷积的权重为很小的值，Bias 为 0
        nn.init.normal_(self.flow_head[-1].weight, mean=0, std=0.001)
        nn.init.constant_(self.flow_head[-1].bias, 0)


    def forward(self, feat_t0, feat_t1):
        x = torch.cat([feat_t0, feat_t1], dim=1)
        x = self.reduce_conv(x) # 调整通道数以匹配 backbone 输入
        x = self.flow_backbone({'spatial_features': x})['spatial_features_2d']
