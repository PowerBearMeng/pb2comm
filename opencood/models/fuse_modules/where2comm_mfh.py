...
class Where2comm(nn.Module):
    ...
    def forward(self, x, rm, record_len, pairwise_t_matrix, backbone=None, heads=None, blind_spot_mask=None):
        """
        Fusion forwarding.
        ...
        blind_spot_mask : torch.Tensor
            [B, 1, H, W], 0=Visible(Drop), 1=Blind(Keep). 
            Added for blind spot transmission logic.
        ...
        if self.multi_scale:
            ...
            for i in range(self.num_levels):
                ...
                for b in range(B):
                    ...
                    C_curr, H_curr, W_curr = node_features.shape[1:]
            ...
            for b in range(B):
                ...
                # =======================================================
                # 【新增逻辑】: 应用盲区 Mask (Single Scale)
                # =======================================================
                if blind_spot_mask is not None:
                    # 1. 调整 Mask 尺寸 (防止传入尺寸不一致)
                    current_mask = F.interpolate(blind_spot_mask, size=(H, W), mode='nearest')
                    
                    # 2. 仅对邻居特征应用 Mask
                    if N > 1:
                        mask_b = current_mask[b]
                        neighbor_feature[1:] = neighbor_feature[1:] * mask_b
                # =======================================================
