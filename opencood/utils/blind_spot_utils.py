...
def points_to_occupancy_grid(points, x_range, y_range, voxel_size, ground_filter_thre=0.0):
    # 你的原始代码逻辑
    points = points[points[:, 2] > ground_filter_thre]

    W = int((x_range[1] - x_range[0]) / voxel_size[0])
    H = int((y_range[1] - y_range[0]) / voxel_size[1])
    ...
    x_idxs = ((valid_points[:, 0] - x_range[0]) / voxel_size[0]).astype(int)
    y_idxs = ((valid_points[:, 1] - y_range[0]) / voxel_size[1]).astype(int)
...
def compute_precise_visibility(occupancy_grid, start_pos, x_range, y_range, voxel_size):
    ...
    start_x = int((start_pos[0] - x_range[0]) / voxel_size[0])
    start_y = int((start_pos[1] - y_range[0]) / voxel_size[1])
    ...
    ignore_radius_pixel = 3.0 / voxel_size[0] 
...
def get_blind_spot_mask(lidar_points, ego_pose, lidar_range, target_feat_shape, voxel_size):
    """
    生成用于 Mask 特征图的盲区掩码
